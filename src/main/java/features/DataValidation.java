...
public class DataValidation {
    ...
    static Pattern PATTERN_FOR_CAR_NAME = Pattern.compile("^[a-zA-Z\\s]+$");
...
// to validate integer for while loop cases
    ...
    public static boolean is_Previous_Date(String input_Date) 
    {
    	...
    	try
    	{
   	      ...
   	      date1 = LocalDate.of(Integer.parseInt(rt[2]), Integer.parseInt(rt[1]), Integer.parseInt(rt[0]));
	       ...
	       date2 = LocalDate.of(Integer.parseInt(rtp[2]), Integer.parseInt(rtp[1]), Integer.parseInt(rtp[0]));
    	}
    ...
    public static boolean validate_City_Name(String city_Name) {
       ...
       boolean str = PATTERN_NAME_FOR_CITY.matcher(city_Name).matches();
		...
		} catch (Exception e) 
        {
			...
			System.out.println("unable intialize data");
		}
        if(check==false)
        {
        ...
        Set<String> sug=new HashSet<String>(suggestions);
        if (!sug.isEmpty())
        {
            
           Iterator ip=sug.iterator();
           while(ip.hasNext())
           {
        ...
        {
        	System.out.println("no suggestions type new word");
        }
        }
        return check;
        }
